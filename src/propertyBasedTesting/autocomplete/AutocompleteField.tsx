...
export function AutocompleteField(props: Props) {
  ...
  React.useEffect(() => {
    const runQuery = async () => {
      ...
      if (
        ...
        // eg.: We receive AZ while we already have results for AZE
        return
      }
    ...
    <div>
      ...
      <ul>
        {searchResults
          ...
          .filter(r =>
            props.enableBugUnfilteredResults ? true : r.startsWith(query),
          )
          .map(r => (
            <li key={r}>{r}</li>
          ))}
